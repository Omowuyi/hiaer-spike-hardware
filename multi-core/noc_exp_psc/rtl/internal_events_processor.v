...
// State machine
// The state register drives the enable and reset of new_row_b, which is
// sixteen groups of seventy-two bits, so one bit fans out to over a thousand
// flip-flops spread across the core. That fanout is what makes the critical
// path 77 per cent routing. MAX_FANOUT tells the tool to replicate the
// register so each cluster of destinations has a local copy; the logic is
// unchanged and so is the behaviour.
(* MAX_FANOUT = 64 *) reg [3:0] curr_state;
reg [3:0] next_state;
