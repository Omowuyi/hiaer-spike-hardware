...
module switch_32_1(
    ...
    generate
        ...
        for(j=0; j<2; j=j+1) begin
            axis_switch_16_1 #(.N(16), .DW(512)) in_switch(
                .aclk(m.aclk),
                .aresetn(m.aresetn),
                ...
                // axis_switch_16_1 has no s_req_suppress -- its round-robin
                // arbiter is plain RTL with no suppress path.  tlast is carried
                // but unused here, so tie the inputs and leave the output open.
                .s_axis_tlast(16'b0),
                .m_axis_tlast()
            );
        end
