...
// =========================================================================
// Synapse Opcode Detection (per 32-bit entry in 256-bit HBM read)
//   LOCAL     = 3'b000: weight accumulation (IEP handles via exec_hbm_rdata)
//   INTER_CORE= 3'b001: inter-core relay spike (NEW - route via NoC)
//   SPIKE_OUT = 3'b100: spike output marker (existing - write to spike FIFO)
//
// INTER_CORE synapse format: [31:29]=001, [28:25]=dst_core(4b),
//                            [24:8]=relay_addr(17b), [7:0]=reserved
// SPIKE_OUT  synapse format: [31:29]=100, [28:17]=zeros, [16:0]=spike_addr(17b)
// =========================================================================

// INTER_CORE detection: opcode[2:0] == 3'b001 means bits [31:29] = 001
wire spk0_is_inter = (hbm_rdata[031:029] == 3'b001);
wire spk1_is_inter = (hbm_rdata[063:061] == 3'b001);
wire spk2_is_inter = (hbm_rdata[095:093] == 3'b001);
wire spk3_is_inter = (hbm_rdata[127:125] == 3'b001);
wire spk4_is_inter = (hbm_rdata[159:157] == 3'b001);
wire spk5_is_inter = (hbm_rdata[191:189] == 3'b001);
wire spk6_is_inter = (hbm_rdata[223:221] == 3'b001);
wire spk7_is_inter = (hbm_rdata[255:253] == 3'b001);

// Spike FIFO data: SPIKE_OUT uses [16:0], INTER_CORE uses [24:8] (relay addr)
assign spk0_din  = spk0_is_inter ? hbm_rdata[024:008] : hbm_rdata[016:000];
assign spk1_din  = spk1_is_inter ? hbm_rdata[056:040] : hbm_rdata[048:032];
assign spk2_din  = spk2_is_inter ? hbm_rdata[088:072] : hbm_rdata[080:064];
assign spk3_din  = spk3_is_inter ? hbm_rdata[120:104] : hbm_rdata[112:096];
assign spk4_din  = spk4_is_inter ? hbm_rdata[152:136] : hbm_rdata[144:128];
assign spk5_din  = spk5_is_inter ? hbm_rdata[184:168] : hbm_rdata[176:160];
assign spk6_din  = spk6_is_inter ? hbm_rdata[216:200] : hbm_rdata[208:192];
assign spk7_din  = spk7_is_inter ? hbm_rdata[248:232] : hbm_rdata[240:224];

// Spike FIFO write: trigger on SPIKE_OUT (bit 31=1) OR INTER_CORE (opcode=001)
assign spk0_wren = !spk0_full & exec_hbm_rx_phase1_done & exec_hbm_rvalidready_2x & (hbm_rdata[031] | spk0_is_inter);
assign spk1_wren = !spk1_full & exec_hbm_rx_phase1_done & exec_hbm_rvalidready_2x & (hbm_rdata[063] | spk1_is_inter);
assign spk2_wren = !spk2_full & exec_hbm_rx_phase1_done & exec_hbm_rvalidready_2x & (hbm_rdata[095] | spk2_is_inter);
assign spk3_wren = !spk3_full & exec_hbm_rx_phase1_done & exec_hbm_rvalidready_2x & (hbm_rdata[127] | spk3_is_inter);
assign spk4_wren = !spk4_full & exec_hbm_rx_phase1_done & exec_hbm_rvalidready_2x & (hbm_rdata[159] | spk4_is_inter);
assign spk5_wren = !spk5_full & exec_hbm_rx_phase1_done & exec_hbm_rvalidready_2x & (hbm_rdata[191] | spk5_is_inter);
assign spk6_wren = !spk6_full & exec_hbm_rx_phase1_done & exec_hbm_rvalidready_2x & (hbm_rdata[223] | spk6_is_inter);
assign spk7_wren = !spk7_full & exec_hbm_rx_phase1_done & exec_hbm_rvalidready_2x & (hbm_rdata[255] | spk7_is_inter);


// Backpressure: stall HBM reads if ANY spike FIFO is full during phase 2
wire any_spk_full = spk0_full | spk1_full | spk2_full | spk3_full |
...
// TIMING FIX: Pipeline exec_hbm_rvalidready by 1 cycle.
// Both halves of exec_hbm_rdata are now registered (latched on count=0 and count=1).
// The valid signal must be delayed by 1 cycle so the IEP reads the registered data
// at the correct time. Data and valid remain aligned - just shifted by 1 clock.
// IEP/EEP/PFC behavior is unchanged (they wait for valid, read data, process).
wire exec_hbm_rvalidready_pre = hbm_rvalid & hbm_rready & hbm_count & ~hbmFIFO_full;
reg exec_hbm_rvalidready_pipe;
always @(posedge clk) begin
    if (~resetn)
        exec_hbm_rvalidready_pipe <= 1'b0;
    else
        exec_hbm_rvalidready_pipe <= exec_hbm_rvalidready_pre;
end
assign exec_hbm_rvalidready = exec_hbm_rvalidready_pipe;
...
// =========================================================================
// INTER_CORE Filter: Zero out INTER_CORE (opcode 001) entries in the data
// sent to IEP. Without this, the IEP would accumulate the INTER_CORE
// synapse weight at whatever address is in the entry, corrupting local
// URAM neuron data. LOCAL (000) and SPIKE_OUT (100) are untouched.
//
// SAFETY: Existing tests use ONLY LOCAL and SPIKE_OUT synapses.
// INTER_CORE (001) never appears in existing synapse tables.
// This filter has ZERO effect on any existing test or DVS model.
// =========================================================================
wire [255:0] hbm_rdata_filtered;
genvar fi;
generate
    for (fi = 0; fi < 8; fi = fi + 1) begin : gen_inter_core_filter
        wire [2:0] entry_opcode = hbm_rdata[fi*32+31 : fi*32+29];
        // Zero out the entire 32-bit entry if opcode = INTER_CORE (001)
        // This prevents the IEP from processing it as a LOCAL synapse
        assign hbm_rdata_filtered[fi*32+31 : fi*32] =
            (entry_opcode == 3'b001) ? 32'b0 : hbm_rdata[fi*32+31 : fi*32];
    end
endgenerate

// TIMING FIX: Both halves are now REGISTERED (no combinational path to IEP).
// The INTER_CORE filter (hbm_rdata_filtered) is applied BEFORE registration,
// so the comparator+mux logic is absorbed into the register's setup time
// instead of adding to the IEP's combinational input path.
// exec_hbm_rvalidready is delayed by 1 cycle to maintain data/valid alignment.
...
always @(posedge clk) begin
    if (~resetn) begin
        ...
        hbm_count <= ~hbm_count;
        if(~hbm_count) begin
            // Latch filtered version for lower half (count=0, 1st read)
            hbm_rdata_lower <= hbm_rdata_filtered;
        end
        else begin
            // Latch filtered version for upper half (count=1, 2nd read)
            hbm_rdata_upper <= hbm_rdata_filtered;
        end
    end
end
//// for debugging
