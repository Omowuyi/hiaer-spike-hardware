...
module pcie_tdest_generator(
    ...
    // MULTICORE ROUTING: Beat counter + registered tdest
    // Each uint64 element = 8 DMA bytes. 64-element command = 512 bytes = 8 beats.
    // coreID at element 62 -> raw byte 496 -> beat 7, offset 48 -> tdata[391:384].
    // Beat counter cycles 0-7. On beat 7, latch coreID from tdata[387:384].
    reg [2:0] beat_count;
    reg [3:0] core_select_reg;

    always @(posedge s.aclk) begin
        if (!s.aresetn) begin
            beat_count <= 3'd0;
            core_select_reg <= 4'd0;
        end else if (s.tvalid) begin
            beat_count <= beat_count + 3'd1;
            if (beat_count == 3'd7)
                core_select_reg <= s.tdata[387:384];
        end
    end

    assign m.tdest = {1'b0, core_select_reg};
