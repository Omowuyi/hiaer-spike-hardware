...
generate 
    for (j=0; j<16;j=j+1) begin
        assign exec_hbm_rdata_reg_signext[j] = (exec_hbm_rdata_reg_arr[j][15]) ? {19'hFFFFF, exec_hbm_rdata_reg_arr[j]} : {19'h00000, exec_hbm_rdata_reg_arr[j]};
    end
endgenerate
...
// Feature 1e: Shadow URAM offset for axonal delay delivery
reg      [3:0] shadow_uram_offset_param;
reg            legacy_noise_en_param;
// Axonal delay register (single pending delayed spike)
reg        delay_pending;
reg [12:0] delay_addr;
reg [3:0]  delay_group;
...
// Axonal delay combinational signals
reg delay_add_pending;
reg [12:0] delay_add_addr_comb;
reg [3:0] delay_add_group_comb;
reg dm_upper, dm_lower;

// Axonal delay sequential management
always @(posedge clk) begin
    if (~resetn) begin
        delay_pending <= 1'b0;
        delay_addr <= 13'd0;
        ...
        if (exec_run && delay_pending && delay_counter > 4'd0)
            delay_counter <= delay_counter - 4'd1;
        if (delay_add_pending && !delay_pending && !exec_uram_phase0_done) begin
            delay_pending <= 1'b1;
            delay_addr <= delay_add_addr_comb;
            delay_group <= delay_add_group_comb;
            delay_counter <= delay_value_param;
        end
        if (delay_pending && delay_counter == 4'd0 && exec_uram_phase0_done)
            delay_pending <= 1'b0;
    end
end
...
always @(*) begin
    for (i = 0; i < NEURON_GROUPS; i=i+1) begin
        ...
        // L6d noise behavior for all modes (shift=-17 disables noise)
        // legacy_noise_en_param controls only 35/32-bit Phase 0 and Phase 2
...
always @(*) begin
    ...
    if (curr_state==STATE_INIT_URAM) begin
        ...
        for (i = 0; i < NEURON_GROUPS; i=i+1) begin
            // Compute delay match for this neuron group
            // TIMING FIX: Use pre-registered match (absorbs 13-bit compare into register)
            ...
            if (uram_waddr[i][0]) begin
                // ODD address - upper half-word
                if (legacy_noise_en_param) begin
                    // ===== 35-BIT MP MODE (2024 behavior) =====
                    if ((uram_rmwmem_upper[i] > $signed(threshold_param[34:0])) && ({uram_waddr[i], i[3:0]} < num_outputs)) begin
                        if (soft_reset_en_param)
                            uram_wdata[i] = {1'b1, uram_rmwmem_upper[i] - $signed(threshold_param[34:0]), uram_rmwdata_lower[i]};
                        else
                            uram_wdata[i] = {1'b1, 35'd0, uram_rmwdata_lower[i]};
                    end else begin
                        if (exec_neuron_model_param==2'd0) begin
                            uram_wdata[i] = {1'b0, 35'd0, uram_rmwdata_lower[i]};
                        end else if (exec_neuron_model_param==2'd1) begin
                            uram_wdata[i] = {1'b0, uram_rmwmem_upper[i] + 35'sd1, uram_rmwdata_lower[i]};
                        end else if (exec_neuron_model_param==2'd2) begin
                            uram_wdata[i][71] = 1'b0;
                            uram_wdata[i][70:36] = prbs_shift_signext[i][34:0] + uram_rmwmem_upper[i] - (uram_rmwmem_upper[i] >>> leak_param);
                    ...
                    // ===== 32-BIT MP MODE (refractory/delay) =====
                    if (dm_upper) begin
                        uram_wdata[i] = {1'b1, 3'd0, 32'd0, uram_rmwdata_lower[i]};
                    end else if (refrac_ctr_upper[i] > 3'd0) begin
                        ...
                        if (dual_synapse_en_param && shadow_uram_offset_param != 4'd0) begin
                            delay_add_pending = 1'b1;
                            delay_add_addr_comb = uram_waddr[i] + {9'd0, shadow_uram_offset_param};
                            delay_add_group_comb = i[3:0];
                        end
                    end else begin
                        if (exec_neuron_model_param==2'd0) begin
                ...
                // EVEN address - lower half-word
                if (legacy_noise_en_param) begin
                    // ===== 35-BIT MP MODE (2024 behavior) =====
                    if ((uram_rmwmem_lower[i] > $signed(threshold_param[34:0])) && ({uram_waddr[i], i[3:0]} < num_outputs)) begin
                        if (soft_reset_en_param)
                            uram_wdata[i] = {uram_rmwdata_upper[i], 1'b1, uram_rmwmem_lower[i] - $signed(threshold_param[34:0])};
                        else
                            uram_wdata[i] = {uram_rmwdata_upper[i], 1'b1, 35'd0};
                    end else begin
                        if (exec_neuron_model_param==2'd0) begin
                            uram_wdata[i] = {uram_rmwdata_upper[i], 1'b0, 35'd0};
                        end else if (exec_neuron_model_param==2'd1) begin
                            uram_wdata[i] = {uram_rmwdata_upper[i], 1'b0, uram_rmwmem_lower[i] + 35'sd1};
                        end else if (exec_neuron_model_param==2'd2) begin
                            uram_wdata[i][71:36] = uram_rmwdata_upper[i];
                            uram_wdata[i][35] = 1'b0;
                            uram_wdata[i][34:0] = prbs_shift_signext[i][34:0] + uram_rmwmem_lower[i] - (uram_rmwmem_lower[i] >>> leak_param);
                        end else if (exec_neuron_model_param==2'd3) begin
                    ...
                    // ===== 32-BIT MP MODE (refractory/delay) =====
                    if (dm_lower) begin
                        uram_wdata[i] = {uram_rmwdata_upper[i], 1'b1, 3'd0, 32'd0};
                    end else if (refrac_ctr_lower[i] > 3'd0) begin
        ...
        for(i=0;i < NEURON_GROUPS; i=i+1) begin
            // Phase2: accumulate HBM weights into MP
            if (legacy_noise_en_param) begin
                // ===== 35-BIT MP MODE: accumulate into full [34:0] =====
                if (uram_waddr[i][0]) begin
                    uram_wdata[i] = {{1'b0, uram_rmwmem_upper[i] + exec_hbm_rdata_reg_signext[i][34:0]}, uram_rmwdata_lower[i]};
                end else begin
                    uram_wdata[i] = {uram_rmwdata_upper[i], {1'b0, uram_rmwmem_lower[i] + exec_hbm_rdata_reg_signext[i][34:0]}};
                end
            end else begin
                // ===== 32-BIT MP MODE: accumulate into [31:0], preserve refrac_ctr =====
                if (uram_waddr[i][0]) begin
