...
module command_interpreter #(
  ...
  // input [16:0]            num_inputs,          // number of inputs/axons (Now moved to PCIe command)
   ...
   // NoC Routing Table Configuration
   output reg        route_cfg_valid,
   output reg [7:0]  route_cfg_addr,
   output reg [9:0]  route_cfg_data
);
...
localparam [7:0] CMD_ROUTE_TABLE_W = 8'd13;     // Write NoC routing table entry: [17:10]=addr, [9:0]=data
...
// =========================================================================
// NoC Routing Table Configuration (CMD_ROUTE_TABLE_W = 0x0D)
// One-cycle pulse on route_cfg_valid when command arrives
// =========================================================================
always @(posedge aclk) begin
    if (!aresetn) begin
        route_cfg_valid <= 1'b0;
        route_cfg_addr  <= 8'd0;
        route_cfg_data  <= 10'd0;
    end else begin
        route_cfg_valid <= 1'b0;  // default: deassert every cycle
        if (rx_curr_state == RX_STATE_IDLE && !rxFIFO_empty && rx_command == CMD_ROUTE_TABLE_W) begin
            route_cfg_valid <= 1'b1;
            route_cfg_addr  <= rxFIFO_dout[17:10];
            route_cfg_data  <= rxFIFO_dout[9:0];
        end
    end
end
         ...
         if (!rxFIFO_empty) begin
            ...
            case (rx_command)
              ...
              CMD_NTWK_PARAM_W: begin
               ...
               // NoC routing table write (sequential logic handled in separate always block)
               CMD_ROUTE_TABLE_W: begin
                  rxFIFO_rden = 1'b1;
                  rx_next_state = RX_STATE_IDLE;
               end
               default: begin
