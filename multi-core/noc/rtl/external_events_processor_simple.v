...
always @(posedge clk) begin
    if (~resetn) begin
        ...
        exec_eep_phase3_done  <= 1'b0;
        microphase_ctr <= 4'b0;
    end else if (next_state == STATE_READ_INPUTS) begin
        exec_eep_phase1_ready <= 1'b1;
        // Assert phase3_done when transitioning from Phase 3 to read inputs
        if (curr_state == STATE_PHASE3_NOC)
            exec_eep_phase3_done <= 1'b1;
    end else if (next_state == STATE_WAIT_IEP_PHASE2) begin
        exec_eep_phase1_ready <= 1'b0;
        exec_eep_phase1_done  <= 1'b0;
